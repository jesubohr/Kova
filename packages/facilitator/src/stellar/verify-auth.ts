 ...
 */
export async function verifyAuthEntry(opts: VerifyAuthEntryOptions): Promise<VerifyAuthEntryResult> {
  ...
  const actualContractId = StrKey.encodeContract(contractArgs.contractAddress().contractId() as unknown as Buffer)
