...
function getServer(network: "testnet" | "mainnet" = "testnet"): Horizon.Server {
  return new Horizon.Server(network === "mainnet" ? HORIZON_MAINNET : HORIZON_TESTNET)
}
...
export async function getStellarUSDCBalance(
  ...
  try {
    ...
    const usdcBalance = account.balances.find(
      (b) => b.asset_type === "credit_alphanum4" && (b as Horizon.HorizonApi.BalanceLineAsset).asset_code === "USDC",
...
export function getStellarExplorerAddressUrl(address: string, network: "testnet" | "mainnet" = "testnet"): string {
  const base = network === "mainnet" ? EXPLORER_MAINNET : EXPLORER_TESTNET
  return `${base}/account/${address}`
}

export function getStellarExplorerTxUrl(txHash: string, network: "testnet" | "mainnet" = "testnet"): string {
  const base = network === "mainnet" ? EXPLORER_MAINNET : EXPLORER_TESTNET
