...
describe("KovaClient", () => {
  ...
  it("auto-pays 402 and retries with X-PAYMENT header", async () => {
    ...
    expect(mockFetch.mock.calls[1][1].headers["X-PAYMENT"]).toBe("base64encodedpayment")
  })
  ...
  it("throws BudgetExceededError when maxPerRequest exceeded", async () => {
    ...
    await expect(client.fetch("https://api.example.com/expensive")).rejects.toThrow(BudgetExceededError)
  })
  ...
  it("fetchAll sends concurrent requests", async () => {
    ...
    const results = await client.fetchAll(["https://api.example.com/a", "https://api.example.com/b"])
