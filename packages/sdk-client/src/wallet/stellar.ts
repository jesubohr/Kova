import { Keypair, xdr, StrKey, Address, Networks, nativeToScVal, authorizeInvocation, rpc } from "@stellar/stellar-sdk"
...
export function createStellarWallet(config: WalletConfig): StellarWallet {
  ...
  async function signAuthEntry(opts: SignAuthEntryOptions): Promise<SignedAuthEntry> {
    ...
    const entry = await authorizeInvocation(keypair, validUntil, invocation, undefined, networkPassphrase)
